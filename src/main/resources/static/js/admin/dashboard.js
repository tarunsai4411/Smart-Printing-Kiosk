...
async function loadDashboard() {
  ...
  const paid = jobs.filter(
    (j) => (j.paymentStatus || "").toUpperCase() === "PAID"
  );
  const unpaid = jobs.length - paid.length;
  const revenue = paid.reduce((s, j) => s + Number(j.amount || 0), 0);
  ...
  paymentChart = new Chart(document.getElementById("paymentChart"), {
    ...
    data: {
      labels: ["Paid", "Pending"],
      datasets: [{ data: [paid.length, unpaid] }],
    },
  });
}
loadDashboard();
